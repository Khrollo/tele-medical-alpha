...
// Workflow chips are only clinically meaningful for recent activity.
// This query runs on the 3s waiting-room polling path, so scanning a
// patient's full visit history + JSONB notes every tick is wasteful.
// 60 days covers routine follow-up windows while keeping row/payload
// counts small enough that the poll stays cheap.
const WORKFLOW_LOOKBACK_DAYS = 60;
...
export async function getWorkflowFlagsForPatients(
  ...
  lookbackFloor.setDate(lookbackFloor.getDate() - WORKFLOW_LOOKBACK_DAYS);
  ...
  const visitRows = await db
    ...
    .where(
      and(
        ...
        gte(visits.createdAt, lookbackFloor),
      ),
    )
